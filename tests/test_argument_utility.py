...
from d3rlpy.preprocessing.observation_scalers import MinMaxObservationScaler
...
@pytest.mark.parametrize("value", ["min_max", MinMaxObservationScaler(), None])
def test_check_observation_scaler(value):
    scaler = check_observation_scaler(value)
    if value is None:
