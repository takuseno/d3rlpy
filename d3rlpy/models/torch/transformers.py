...
class CausalSelfAttention(nn.Module):  # type: ignore
    ...
    def forward(self, x: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        assert attention_mask.dim() == 2, f"Expects (B, T), but got {attention_mask.shape}"
        ...
        attention = attention + attention_mask.view(batch_size, 1, 1, context_size) * -10000
...
class Block(nn.Module):  # type: ignore
    ...
    def forward(self, inpt: tuple[torch.Tensor, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
...
class ContinuousDecisionTransformer(nn.Module):  # type: ignore
    ...
    def forward(
        ...
        # (B, 3, T) -> (B, T, 3) -> (B, T * 3)
        attention_mask = attention_mask.transpose(1, 2).reshape(batch_size, 3 * context_size)
