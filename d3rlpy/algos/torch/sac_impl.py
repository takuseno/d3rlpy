...
class DiscreteSACImpl(TorchImplBase):
    def __init__(self, observation_shape, action_size, actor_learning_rate,
                 critic_learning_rate, temp_learning_rate, gamma, n_critics,
                 bootstrap, share_encoder, initial_temperature, eps,
                 use_batch_norm, q_func_type, use_gpu, scaler, augmentation,
                 n_augmentations, encoder_params):
    ...
    def update_critic(self, obs_t, act_t, rew_tp1, obs_tp1, ter_tp1):
        ...
        loss = compute_augemtation_mean(
            self.augmentation, self.n_augmentations, self._compute_critic_loss,
            {
                'obs_t': obs_t,
                'act_t': act_t.long(),
                'rew_tp1': rew_tp1,
